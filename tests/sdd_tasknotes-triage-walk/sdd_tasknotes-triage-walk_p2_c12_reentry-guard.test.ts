...
describe("TaskTriageFeature.beginWalk — re-entry guard", () => {
	it("rejects starting a second walk while one is already running", async () => {
		const listTasks = vi.fn(async () => [task({ path: "a.md", title: "A" }), task({ path: "b.md", title: "B" })]);
