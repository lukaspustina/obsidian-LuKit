...
describe("TaskTriageFeature.handleComplete — non-recurring task", () => {
	it("calls bridge.complete once, not toggleCompleteInstance, and advances the walk", async () => {
		const bridge = fakeBridge({
			listTasks: vi.fn(async () => [
				task({ path: "A.md", due: "2026-07-01" }),
				task({ path: "B.md", due: "2026-07-01" }),
			]),
		});
		...
		expect(bridge.complete).toHaveBeenCalledWith("A.md");
