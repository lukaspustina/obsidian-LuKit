...
describe("TaskTriageFeature.handleComplete — recurring task", () => {
	it("calls bridge.toggleCompleteInstance(path, today) and not bridge.complete", async () => {
		const toggleCompleteInstance = vi.fn(async () => undefined);
		const complete = vi.fn(async () => undefined);
		const { internals } = setup(fakeBridge({ toggleCompleteInstance, complete }));

		const recurringTask = task({ path: "R.md", isRecurring: true, scheduled: "2026-07-02" });
		internals.walkActive = true;
		internals.tasks = [recurringTask];
		internals.index = 0;

		await internals.handleComplete();

		expect(toggleCompleteInstance).toHaveBeenCalledTimes(1);
