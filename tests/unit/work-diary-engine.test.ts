...
describe("ensureTodayHeader", () => {
	...
	it("works with real note content (frontmatter + pinned links + diary separator)", () => {
		const content = [
			...
			"---",
			"",
			'**[[Projekte/Team/Notizen.md|Notizen: Beispielprojekt 2025]]**',
			"",
			"---",
			"**Fr, 06.02.2026 -- Musterstadt**",
			"- [[Vorgang - Acme]]: Abstimmung mit Erika Beispiel, 06.02.2026",
		].join("\n");
		const result = ensureTodayHeader(content, "de", friday);
		const lines = result.newContent.split("\n");
		...
		expect(lines[10]).toBe("**Fr, 06.02.2026 -- Musterstadt**");
	});
...
describe("formatDiaryEntry", () => {
	...
	it("strips wikilinks from a pure-wikilink heading (no trailing text)", () => {
		const heading = "[[Besprechung - Introduction M. Müller M. Richardson M. Mustermann, 02.03.2026]]";
		const result = formatDiaryEntry("Vorgang Note", heading);
		expect(result).not.toContain("[[Besprechung");
		expect(result).toBe(
			"- [[Vorgang Note#Besprechung - Introduction M. Müller M. Richardson M. Mustermann, 02.03.2026|Vorgang Note: Besprechung - Introduction M. Müller M. Richardson M. Mustermann, 02.03.2026]]"
		);
	});
