...
describe("isStandaloneBold", () => {
	...
	it("matches bold with special characters", () => {
		expect(isStandaloneBold("**Besprechung: Acme, 15.03.2025**")).toEqual({
			inner: "Besprechung: Acme, 15.03.2025",
		});
	});
});
...
describe("addFrontmatterTag", () => {
	...
	it("preserves other frontmatter fields", () => {
		...
		expect(lines[2]).toBe("Author: Max");
...
describe("migrateVorgangNote", () => {
	it("migrates a full old-format note with all steps", () => {
		...
		expect(newContent).toContain("##### Abstimmung mit Erika, 01.02.2026");
		...
		expect(newContent).not.toContain("**Abstimmung mit Erika, 01.02.2026**");
