...
function setup(bridge: MailBridge) {
	...
	// _resources already exists for the Vorgang's folder — the existence check
	// should short-circuit mkdir.
	app.vault.adapter.folders.add("Vorgänge/_resources");
...
describe("SDD email-attachments p2 c9", () => {
	it("does not call mkdir when _resources already exists, but still saves and links the attachment", async () => {
