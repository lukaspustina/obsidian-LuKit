...
describe("sdd_erinnerungen-triage p2 c3: snooze rewrites reminder date", () => {
	it("rewrites the reminder date to tomorrow and advances the walk", async () => {
		const { app, internals } = setup(fakeBridge());

		await internals.beginWalk();
		await internals.handleSnooze("tomorrow" as SnoozeKind);

		const content = app.vault.files.get("Diary.md");
		expect(content).toContain("- Zahnarzt anrufen, 03.07.2026");
		expect(content).not.toContain("- Zahnarzt anrufen, 01.07.2026");
