...
describe("sdd_erinnerungen-triage p2 c8: kein Tagebuch-Pfad", () => {
	it("baut nur Task-Stops und erwähnt kein Tagebuch in Notices", async () => {
		const bridge = fakeBridge({ listTasks: vi.fn(async () => [task()]) });
		const { internals } = setup(bridge, null);
