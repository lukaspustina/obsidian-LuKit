...
describe("SDD erinnerungen-triage Phase 2 #9: beide Quellen leer -> Notice, kein Walk", () => {
	it("GIVEN Tagebuch ohne fällige Erinnerungen, listTasks=[] WHEN beginWalk THEN Notice 'Keine fälligen Tasks oder Erinnerungen', walkActive=false, presentStop nicht aufgerufen", async () => {
		const noDueDiary = [
			"---",
			"tags: []",
			"---",
		...
		const { internals } = setup(bridge, noDueDiary);

		await internals.beginWalk();

		expect(lastNotice()).toContain("Keine fälligen Tasks oder Erinnerungen");
