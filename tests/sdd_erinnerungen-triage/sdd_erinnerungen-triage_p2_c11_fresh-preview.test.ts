...
describe("sdd_erinnerungen-triage p2 c11: fresh preview never caches reminders", () => {
	it("reads reminder previews fresh after a mutation", async () => {
		...
		expect(before).toContain("- Zahnarzt anrufen, 01.07.2026");
		expect(before).toContain("# Erinnerungen");

		expect(after).toContain("# Erinnerungen");
		expect(after).toContain("- Angebot prüfen");
		expect(after).not.toContain("- Zahnarzt anrufen, 01.07.2026");
	});
});
