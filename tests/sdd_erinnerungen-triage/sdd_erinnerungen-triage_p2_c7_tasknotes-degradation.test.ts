...
describe("SDD erinnerungen-triage Phase 2 #7: TaskNotes degradation still walks reminders", () => {
	it("GIVEN DIARY, TaskNotes api-missing WHEN beginWalk THEN nur Erinnerungs-Stops, TaskNotes-Notice, listTasks nicht aufgerufen", async () => {
		const bridge = fakeBridge({
			availability: vi.fn(() => ({ ok: false, reason: "api-missing" } as const)),
		});
		const { internals } = setup(bridge, DIARY);
