...
**Next-steps intake:** filing a Besprechung or an email thread into a Vorgang (see below) doesn't just archive it — it also drops the action items into an inbox-like area inside `# Nächste Schritte`, below a `#### Unsortiert` marker, so they don't get buried in the dated archive sections. Each filed source gets its own group (`- Aus [[Quelle]]`) with its items as sub-bullets. Meeting notes that group their next steps under a `<Name>:` line keep that grouping: the name becomes a parent bullet and its items sit beneath it. An item written as `<name>: text`, or a whole block under a `<name>:` line, for someone other than you is set apart under a `- Warte auf:` line — who counts as "you" is configured via *Eigene Namen*, and while that setting is empty the split is off and everything counts as yours. Groups pile up oldest-first, since this area is meant to be worked off, not read top-down. **Vorgänge: Fällige Aufgaben durchgehen** (see Task Triage) is how you work through them. A note is one stop, and **⌘S** opens all of its due groups at once — one section per group, every line with a checkbox and an editable field, so wording can be fixed on the way out; anything you leave unticked stays in the group, emptying a line's field deletes it, and a discard box throws away a whole group. Sorting the intake is a step inside working a case, not the end of it: you always land back on the case afterwards — to set its dates, or to keep going through the rest. Since a Vorgang note is itself a task, the same stop completes it (**⌘D**), moves it (**⌘1/⌘2/⌘3/⌘T**) or sets both its dates (**⌘G**, both prefilled, an emptied field clears the date) without ending the stop.
