...
Filing an email captures the **whole conversation**: your Sent replies **and** the thread's other emails still in your inbox are pulled in alongside the received message and written as one section, newest-first, de-duplicated against what the Vorgang already contains (re-filing a thread only adds new messages). The sibling inbox emails are archived too, so the whole thread leaves the inbox. In the preview each message has an **include/exclude checkbox** and an **editable body**, the generated section title can be edited in place, and **„Ablegen und Öffnen“** files and then opens the target note in the current window — excluding one keeps it out of the note but still archives it. Suggestions also learn across sessions from Vorgänge you've filed into before.
