...
| ⌘D = complete the task | ⌘D = "take over the whole group", as at today's intake stop | With the note as the object, "erledigt" is the only meaning that matches the other stop kinds. Taking everything over is ⌘S + Enter, since all checkboxes are preselected. |
