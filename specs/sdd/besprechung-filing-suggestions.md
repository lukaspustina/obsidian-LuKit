...
**REQ-11** When `suggestions` is absent or an empty array, the modal's item list and ordering shall be byte-for-byte unchanged from current behaviour (sentinels, then mtime-sorted notes) and no pinned rows shall exist.

### Feature wiring

**REQ-12** `BesprechungFeature` shall build the filing corpus by scanning all `.md` files under the configured `settings.besprechung.folderPath` whose frontmatter carries a `filed_into` value. For each, it shall produce a `FiledRecord` with: `rawTitle` = frontmatter `title` (falling back to the file `basename`); `target` = the `filed_into` wikilink target reduced to its bare basename — take the inner text of `[[…]]`, drop any `|alias` suffix, then take the last `/`-separated path segment, with no `.md` extension (e.g. `"[[Vorgang - X|Alias]]"` → `"Vorgang - X"`, `"[[folder/Vorgang - X]]"` → `"Vorgang - X"`); and `filedAt` = `Date.parse(filed_at)` when present and finite, else `null`. The besprechung currently being filed shall be excluded from its own corpus.

**REQ-13** For both `filePendingCmd` (per besprechung in the walk) and `fileActiveBesprechungCmd`, `BesprechungFeature` shall compute `suggestFilingTargets(candidateTitle, corpus, candidateBasenames, { now: Date.now() })` — where `candidateTitle` is the besprechung's frontmatter `title` or its `basename`, and `candidateBasenames` is the selectable section-note set (the same set the modal lists) — and pass the resulting ordered basenames as `suggestions` to `SectionNoteSuggestModal`.
