...
- Command `task-triage-walk` ("Vorgang: Triage due tasks"). Walk order: `scheduled` ASC, then `due` ASC, dateless last per key. Keyboard: ⌘D complete (recurring → `toggleCompleteInstance(today)`, else `complete`), ⌘1/⌘2/⌘3 snooze morgen/+1 Woche/nächster Montag, ⌘T freies Datum (`TaskTriageDateModal`, date-only), ⌘X skip today's instance (recurring only; snooze hidden for recurring, ⌘X hidden for non-recurring), Enter open & stop, Esc skip (deferred-`setTimeout` acted-flag pattern from `section-note-suggest.ts`), ⌘. stop. Summary Notice on every exit path: `Triage beendet: X erledigt, Y verschoben, Z ausgelassen, S übersprungen, R offen` (five buckets sum to total; "visited" = received a mutating action). Mutation failure → Notice, walk stays on the task. Concurrent-walk guard. Preview visible by default: `buildTriagePreview` strips frontmatter, Vorgang-style notes (with `# Fakten und Pointer`) trimmed to that section + newest (topmost) h5 via `extractSection` reuse.
